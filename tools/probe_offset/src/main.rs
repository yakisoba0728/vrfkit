...
impl ReplicationSink for MinimalSink {
    ...
    fn on_actor_close(&mut self, _channel_index: u32, _actor_net_guid: NetworkGuid, _dormant: bool) {}
    fn on_content_block(
        &mut self,
        _channel_index: u32,
...
fn process_file(path: &str) -> Result<(), String> {
    ...
    let mut repl_reader = ReplicationReader::new(branch)
        .map_err(|e| format!("unsupported branch: {e}"))?;
    let mut sink = MinimalSink;

    let mut chunk_iter = ChunkIterator::new(&data, preamble.remaining_offset);
    let mut total_packets: u32 = 0;

    // We also want to capture the raw packet bytes that contain the malformed bunch.
    // The diagnostic says packet_id=0, so it's the very first packet.
    // Let's collect all packets and process them, then examine diagnostics.
    ...
    while let Some(chunk) = chunk_iter.next_chunk().map_err(|e| format!("chunk: {e}"))? {
        ...
        // Process all packets from this chunk
        for pkt_data in &all_packets[total_packets as usize..] {
    ...
    println!("  malformed_content_blocks: {}", stats.malformed_content_blocks);
    ...
    for (idx, diag) in stats.diagnostics.iter().enumerate() {
        if let SkipReason::ContentBitsOverrun { declared_content_bits, available_bits } = &diag.reason {
            println!();
            println!("  --- Diagnostic #{idx} ---");
            println!("    reason: ContentBitsOverrun {{ declared={declared_content_bits}, available={available_bits} }}");
            ...
            if let Some(hdr) = &diag.content_block_header {
                println!("    header: has_rep_layout={}, is_actor={}, object_net_guid={}, is_stably_named={}",
                    hdr.has_rep_layout, hdr.is_actor, hdr.object_net_guid, hdr.is_stably_named);
            }
            println!("    bunch_flags: open={}, reliable={}, partial={}, pkg_map={}, must_mapped={}",
                diag.bunch_flags.b_open, diag.bunch_flags.b_reliable, diag.bunch_flags.b_partial,
                diag.bunch_flags.b_has_package_map_exports, diag.bunch_flags.b_has_must_be_mapped_guids);
            ...
            if pkt_id < all_packets.len() {
                ...
                println!();
                println!("    Extracting raw payload from packet {pkt_id} ({} bytes)...", pkt_data.len());
...
fn extract_and_probe_bunch(
    ...
    loop {
        ...
        if bunch_idx == target_bunch_index {
            ...
            println!("    payload_bit_count: {actual_payload_bits} (expected: {expected_payload_bits})");

            // Extract the payload
            let byte_count = (actual_payload_bits as usize).div_ceil(8);
            let mut payload = vec![0u8; byte_count];
            if reader.copy_bits_to(&mut payload, actual_payload_bits).is_err() {
                println!("    ERROR: cannot copy payload bits");
                return;
            }

            println!("    consumed_at_failure: {consumed_at_failure}, remaining: {remaining_at_failure}");
...
fn do_offset_scan(payload_data: &[u8], payload_bits: u64, consumed_at_failure: u64) {
    ...
    let start = if consumed_at_failure > 20 { consumed_at_failure - 20 } else { 0 };
    let end = (consumed_at_failure + 30).min(payload_bits);
    for off in start..=end {
        let mut r = BitReader::with_bit_len(payload_data, payload_bits);
    ...
    println!();
    println!("    === EXPERIMENT: no-header framing (pure content_bits only) at offsets around failure ===");
    let start = if consumed_at_failure > 5 { consumed_at_failure - 5 } else { 0 };
...
fn dump_blocks_at_offset(payload_data: &[u8], payload_bits: u64, start: u64) {
    let mut reader = BitReader::with_bit_len(payload_data, payload_bits);
    ...
    loop {
        ...
        if is_actor {
            ...
            if content_bits > 0 {
                if (content_bits as u64) > reader.bits_remaining() {
                    println!("      block {block_idx}: actor, rep_layout={has_rep_layout}, content_bits={content_bits} OVERRUN at pos {hdr_end}");
                    break;
                }
        ...
        if is_stably_named {
            ...
            if content_bits > 0 {
                if (content_bits as u64) > reader.bits_remaining() {
                    println!("      block {block_idx}: stably_named obj={obj_guid}, content_bits={content_bits} OVERRUN");
                    break;
                }
                let _ = reader.skip_bits(content_bits as u64);
            }
...
fn try_read_header(reader: &mut BitReader<'_>) -> Result<bool, String> {
    let _has_rep_layout = reader.read_bit().map_err(|e| format!("has_rep_layout: {e}"))?;
    let is_actor = reader.read_bit().map_err(|e| format!("is_actor: {e}"))?;
    if is_actor {
        return Ok(false);
    }

    let obj_guid = reader.read_int_packed().map_err(|e| format!("obj_guid: {e}"))?;
    if obj_guid == 0 {
        // guid 0 is invalid, but the parser doesn't early-return here.
        // The real code continues with is_stably_named check.
    }

    let is_stably_named = reader.read_bit().map_err(|e| format!("is_stably_named: {e}"))?;
    if is_stably_named {
        return Ok(false);
    }
    ...
    let class_guid = reader.read_int_packed().map_err(|e| format!("class_guid: {e}"))?;
    if class_guid == 0 {
        return Ok(true); // invalid class -> treated as deleted
    }

    let use_actor_outer = reader.read_bit().map_err(|e| format!("use_actor_outer: {e}"))?;
    if !use_actor_outer {
        let _outer = reader.read_int_packed().map_err(|e| format!("outer_guid: {e}"))?;
    }
...
fn parse_bunch_header_full(reader: &mut BitReader<'_>) -> Result<BunchHeader, String> {
    ...
    if b_close {
        ...
        let _close_reason = reader.read_serialized_int(15).map_err(|e| format!("closeReason: {e}"))?;
    }
    ...
    let _ch_index = reader.read_int_packed().map_err(|e| format!("ch_index: {e}"))?;
    ...
    if b_partial {
        let _initial = reader.read_bit().map_err(|e| format!("bPartialInit: {e}"))?;
        let _fin = reader.read_bit().map_err(|e| format!("bPartialFinal: {e}"))?;
    }

    // VALORANT-specific bit (always present, always discarded)
    let _valorant_bit = reader.read_bit().map_err(|e| format!("valorant_bit: {e}"))?;

    // Channel name (FName): present when reliable or opening
    if b_reliable || b_open {
        let _is_hardcoded = reader.read_bit().map_err(|e| format!("fname_hardcoded: {e}"))?;
        let _name_index = reader.read_int_packed().map_err(|e| format!("fname_index: {e}"))?;
    }

    // Payload bit count: SerializedInt(16384)
    let payload_bit_count = reader.read_serialized_int(16384).map_err(|e| format!("payload: {e}"))?;
