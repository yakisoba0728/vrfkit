...
| Script | Produces |
|---|---|
| `extract_descriptors.py` | `crates/vrf-decode/src/table.rs` (overlay table 1,249 + 84 handles) |
...
```bash
python tools/apply_type_corrections.py           # apply, then verify (88 corrections)
python tools/apply_type_corrections.py --check   # verify only
```

Those 88 corrections are the whole `EXPECTED` set the script re-verifies; `ADDITIONS` is the
...
currently 64 of them, and every one is admitted on wire evidence written into the
...
python tools/apply_type_corrections.py --check    # 88 corrections present
```
