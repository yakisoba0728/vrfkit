...
```
  Typed:            71.0% (properties + RPC parameters)
```
...
| `checkpoint_fields.parquet` | 78,829 | 202,069 | requires `--checkpoints` |
...
| Script | Produces |
|---|---|
| `extract_descriptors.py` | `crates/vrf-decode/src/table.rs` (overlay table 1,234 + 85 handles) |
...
- **Untyped residual** -- the [`export`](#export) `Typed` is ~71% (denominator
