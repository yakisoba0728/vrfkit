...
- **417 tests** plus a layered validation suite (framing / bytes / decode
  errors / semantics).
...
Work in progress. Currently verified: `cargo test --workspace` **417 passing**,
...
(`tools/extract_descriptors.py`) -- 197 groups, 1,248 entries, 84 handles.
...
`tools/apply_type_corrections.py` (87 corrections, verified with `--check`).
...
|---|---|---|
| `crates/vrf-decode/src/table.rs` | `tools/extract_descriptors.py` then `tools/apply_type_corrections.py` | The overlay table (1,248 entries, 197 groups, 84 handles) and handle table |
