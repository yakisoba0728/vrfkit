...
- **404 tests** plus a layered validation suite (framing / bytes / decode
  errors / semantics).
...
| File | Rows | Bytes |
|---|---|---|
| `fields.parquet` | 1,255,920 | 14,866,041 |
| `movement.parquet` | 1,839,607 | 31,835,557 |
| `actors.parquet` | 3,827 | 87,281 |
| `net_guids.parquet` | 16,167 | 153,606 |
| `events.parquet` | 195 | 11,136 |
| `checkpoint_fields.parquet` | 78,829 | 201,767 |
...
(`tools/extract_descriptors.py`) -- 194 groups, 1,233 entries, 84 handles.
...
```
Decoded OK:   714,070      Decode errors:      0
Raw/Skip:      74,624      Not in table: 197,796
No field name: 17,013      Typed:          72.2%
Effect blobs:  53,908
```
...
`value_*`. Before effect linkage 68.8% were untyped; it is now **37.3%.**
...
`tools/apply_type_corrections.py` (72 corrections, verified with `--check`).
...
|---|---|---|
| `crates/vrf-decode/src/table.rs` | `tools/extract_descriptors.py` then `tools/apply_type_corrections.py` | The overlay table (1,233 entries, 194 groups, 84 handles) and handle table |
