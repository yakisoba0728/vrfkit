...
- **405 tests** plus a layered validation suite (framing / bytes / decode
  errors / semantics).
...
| `checkpoint_fields.parquet` | 78,829 | 201,777 |
...
Work in progress. Currently verified: `cargo test --workspace` **405 passing**,
...
(`tools/extract_descriptors.py`) -- 195 groups, 1,234 entries, 84 handles.
...
`value_*`. Before effect linkage 68.8% were untyped; it is now **37.1%.**
...
|---|---|---|
| `crates/vrf-decode/src/table.rs` | `tools/extract_descriptors.py` then `tools/apply_type_corrections.py` | The overlay table (1,234 entries, 195 groups, 84 handles) and handle table |
